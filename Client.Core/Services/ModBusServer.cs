...
namespace Client.Core.Services
{
	...
	/// </summary>
	public class ModBusServer : IModbusServer, IDisposable
	{
		...
		/// <param name="Interval">Interval time of refresh data from the PLC</param>
			public void Suscribe(int Interval = 1000)
			{
				if (timer != null)
				{
		...
		//Read and Write Bool
			public bool ReadCoil(int ItemID)
			{
				try
				{
					bool[] coils = modbusClient.ReadCoils(ItemID, 10);
					return coils.First();
				}
		...
		public Dictionary<string, List<string>> GetListItems(string value = "")
		{
